...
#define	CX_BORDER		3
#define	CY_BORDER		2
#define CX_BORDER1		30
#define CY_BORDER1		40	
...
CListBoxST::CListBoxST()
{
	...
	//draw check btn flag
	m_bIsCheckBtnOk = FALSE;
	//
	m_bIsDcmListUpdate = FALSE;
...
BEGIN_MESSAGE_MAP(CListBoxST, CListBox)
	...
	//ON_CONTROL_REFLECT_EX(LBN_SELCHANGE,OnReflectedSelchange())
	//}}AFX_MSG_MAP
	ON_CONTROL_REFLECT(LBN_SELCHANGE, &CListBoxST::OnLbnSelchange)
	//ON_CONTROL_REFLECT(LBN_DBLCLK, &CListBoxST::OnLbnDblclk)
END_MESSAGE_MAP()
...
void CListBoxST::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	while(!temp.IsEmpty())
	{
		...
		if(-1 == pos) break;
		temp.Replace(temp.Left(pos+1),APP_NULL_STR);
		//sText= sText - sText.Left(pos);
		scout ++;
	}
	//CRect	csRect(0, 0, lpMeasureItemStruct->itemWidth, lpMeasureItemStruct->itemHeight);
	//pDC->DrawText(sText, -1, csRect, DT_WORDBREAK | DT_EXPANDTABS | DT_CALCRECT); 

	//Modify 8.2
	LOGFONT logfont;
	newFont1.GetLogFont(&logfont);
	SetFont(&newFont1);
	//CSize ft = pDC->GetTextExtent(_T("Test"));
	nHeight = abs(logfont.lfHeight) * scout;
	...
	lpMeasureItemStruct->itemHeight += 20;//CY_BORDER*2
} // End of MeasureItem
...
void CListBoxST::DrawItem(LPDRAWITEMSTRUCT pDIStruct)
{
	...
	COLORREF		crNormal = APP_BK_COLOR1;//GetSysColor(COLOR_WINDOW);
	COLORREF		crSelected = APP_BK_COLOR1;//GetSysColor(COLOR_HIGHLIGHT);
	COLORREF		crText = APP_TEXT_COLOR1;//GetSysColor(COLOR_WINDOWTEXT);
	COLORREF		crColor = APP_TEXT_COLOR1;
	...
	CRect rcRadio = pDIStruct->rcItem;
	//m_rcItem = pDIStruct->rcItem;
	...
	// Calculate rcText
	rcText.left = rcIcon.right + 80;

	// draw check button	add 8.5
	if(m_bIsCheckBtnOk)
	{
		rcRadio.left = rcIcon.right + 10;
		rcRadio.right = rcRadio.left + 30;
		rcRadio.bottom = rcRadio.top + 30;
		pDC->DrawFrameControl(&rcRadio, DFC_BUTTON, DFCS_BUTTONCHECK | (bIsSelected?DFCS_CHECKED:0));
	}
	...
	//Draw the line		add 8.3
	UINT width = 4;
	CPen pen(PS_SOLID,width,COLOR_BK_IN);
	CPoint  ptStart(rcItem.left,rcItem.bottom - width/2);
	pDC->MoveTo(ptStart);
	CPen *oldpen = pDC->SelectObject(&pen);
	CPoint  ptEnd(rcItem.right,rcItem.bottom - width/2);
	pDC->LineTo(ptEnd);
	...
	pDC->DrawText(sText, -1, rcCenteredText, DT_WORDBREAK | DT_EXPANDTABS | lpLBData->nFormat);
 	...
 	// Draw focus rectangle		delete 8.4
...
BOOL CListBoxST::OnReflectedDblclk() 
{
	...
	if (!bOutside)	
	{
		...
		GetParent()->GetParent()->SendMessage(WM_CHANGE_WND,(WPARAM)type_reportinfo_index,(LPARAM)GetParent());
...
bool CListBoxST::ReplaceItemIcon(int nIndex,int nImage)
{
	if(nIndex <= 0 || nImage <= 0) return FALSE;	
...
void CListBoxST::OnLbnSelchange()
{
	// TODO: 在此添加控件通知处理程序代码
